...
export function ValidationBlockedClients() {
  useBreadcrumb(['Validation', 'Blocked clients']);

  const [selected, setSelected] = React.useState<number[]>([]);

  const { data, error, loading, refetch } = usePaginatedApi<Item>(
    '/validation/blocked-clients'
  );
        ...
        data={(data?.items || []).map((item) => ({
          ...
          operation: (
            <UnblockClient
              ...
              onSubmit={async (data) => {
                const result = await validationService.unblockClient(data);
